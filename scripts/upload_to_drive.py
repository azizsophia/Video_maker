...
Auth (pick one, via environment / GitHub Secrets):
  ...
  B) Service account JSON — best for Google Workspace / Shared Drives.
     ...
     @gmail use option A, or upload into a Shared Drive with --shared-drive.
...
def get_credentials():
    """Return Drive credentials from whichever secret set is present."""
    ...
    sys.exit("No Drive credentials found. Set GDRIVE_CLIENT_ID/"
             ...
             "GDRIVE_SA_JSON. See docs/DRIVE_SETUP.md.")
...
def main():
    ...
    ap.add_argument("--name", default=None, help="Drive title (default: basename)")
    ap.add_argument("--mime", default="video/mp4")
    ap.add_argument("--folder", default=os.environ.get("GDRIVE_FOLDER_ID"))
    ap.add_argument("--replace", action="store_true",
                    help="Overwrite an existing file of the same name in the folder.")
    args = ap.parse_args()
